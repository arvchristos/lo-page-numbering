...
Although `.po` files are human readable, they cannot be directly used by gettext. The `msgfmt` tool is used to create gettext compatible catalogs (`.mo` files):
```bash
...
def main(*args):
    ...
    # Operations for undomanager go here
